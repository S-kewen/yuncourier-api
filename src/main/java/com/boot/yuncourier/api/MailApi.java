...
public class MailApi {
    ...
    @ApiOperation(value = "通過應用ID和token發送普通郵件", notes = "批量發送請用英文分號隔開輸入多為收件人,費用:2元/條", produces = "application/json")
//描述方法用途和注意事項
    ...
    @ApiOperation(value = "通過應用ID和token發送html郵件", notes = "批量發送請用英文分號隔開輸入多為收件人,費用:2.5元/條", produces = "application/json")
//描述方法用途和注意事項
    @ApiImplicitParams({//描述方法的参数
            @ApiImplicitParam(name = "applyId", value = "應用ID", dataType = "int", paramType = "query", required = true, example = "0"),
            @ApiImplicitParam(name = "token", value = "應用token", dataType = "String", paramType = "query", required = true, example = "32位token"),
    ...
    public Map<String, Object> htmlMail(String applyId, String token,
        ...
        if (firewallCheck == null) {
            ...
            msg = new String(new BASE64Decoder().decodeBuffer(msg), "UTF-8");
            Mail mail = new Mail();
            mail.setUser_id(software.getUser_id());
            mail.setSoftware_id(software.getId());
            mail.setToken(software.getToken());
    ...
    @ApiOperation(value = "通過應用ID和token發送yunTech流量告警郵件", notes = "批量發送請用英文分號隔開輸入多為收件人,費用:3元/條", produces = "application/json")
//描述方法用途和注意事項
    @ApiImplicitParams({//描述方法的参数
            @ApiImplicitParam(name = "applyId", value = "應用ID", dataType = "int", paramType = "query", required = true, example = "0"),
            @ApiImplicitParam(name = "token", value = "應用token", dataType = "String", paramType = "query", required = true, example = "32位token"),
